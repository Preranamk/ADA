...
{     int i;
    int mobile = find_Moblie(arr,d,num);     int pos = search(arr,num,mobile);     if(d[arr[pos-1]-1]==0)         swap(&arr[pos-1],&arr[pos-2]);     else
        swap(&arr[pos-1],&arr[pos]);     for(int i=0;i<num;i++)
    {
        if(arr[i] > mobile)
        {
            if(d[arr[i]-1]==0)                 d[arr[i]-1] = 1;             else                 d[arr[i]-1] = 0;         }
    }
    for(i=0;i<num;i++)
    {
        printf(" %d ",arr[i]);
...
int factorial(int k)
{     int f = 1;     int i = 0;     for(i=1;i<k+1;i++)
    {         f = f*i;
    }
    return f;
}
int main()
{
 ...
 int arr[num],d[num];
z = factorial(num);
    printf("total permutations = %d",z);     printf("\nAll possible permutations are: \n");     for(i=0;i<num;i++)
    {
 d[i] = 0;
